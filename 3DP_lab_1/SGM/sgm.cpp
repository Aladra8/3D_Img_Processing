...
namespace sgm
{
  SGM::SGM(unsigned int disparity_range, unsigned int p1, unsigned int p2, unsigned int window_height, unsigned window_width) : disparity_range_(disparity_range), p1_(p1), p2_(p2), window_height_(window_height), window_width_(window_width)
  {
    compute_hamming_lut();
  }
  ...
  void SGM::compute_path_cost(int direction_y, int direction_x, int cur_y, int cur_x, int cur_path)
  {
    ...
    else
    {
      ...
      for (int d = 0; d < disparity_range_; d++)
      {
        ...
        { // other disparities
          small_penalty_cost = min(path_cost_[cur_path][cur_y + direction_y][cur_x + direction_x][d + 1],
                                   path_cost_[cur_path][cur_y + direction_y][cur_x + direction_x][d - 1]) +
                               p1_;
        }

        big_penalty_cost = best_prev_cost + p2_;
        path_cost_[cur_path][cur_y][cur_x][d] = prev_cost + findMin(no_penalty_cost, small_penalty_cost, big_penalty_cost) - best_prev_cost;
      }
    }
  }

  unsigned long SGM::findMin(unsigned long a, unsigned long b, unsigned long c)
  {
  ...
  void SGM::aggregation()
  {
    ...
    for (int y = pw_.north; y != pw_.south; y++)
    {
      for (int x = pw_.west; x != pw_.east; x++)
      {
        for (int d = 0; d < disparity_range_; d++)
        {
          ...
          {
            sum = path_cost_[i][y][x][d] + sum;
          }

          aggr_cost_[y][x][d] = sum;
        }
      }
    }
  }
